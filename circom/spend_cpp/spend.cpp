...
uint get_total_signal_no() {return 3791;}

uint get_number_of_components() {return 44;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 3660;}
...
// load src
Fr_sub(&expaux[1],&signalValues[mySignalStart + 4],&signalValues[mySignalStart + 3]); // line circom 37
Fr_mul(&expaux[0],&expaux[1],&signalValues[mySignalStart + 2]); // line circom 37
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
Fr_add(&expaux[0],&signalValues[mySignalStart + 5],&signalValues[mySignalStart + 3]); // line circom 38
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_neg(&expaux[1],&signalValues[mySignalStart + 5]); // line circom 39
Fr_add(&expaux[0],&expaux[1],&signalValues[mySignalStart + 4]); // line circom 39
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 0; i++){
...
Fr_sub(&expaux[3],&signalValues[mySignalStart + 4],&circuitConstants[2]); // line circom 32
Fr_mul(&expaux[1],&signalValues[mySignalStart + 4],&expaux[3]); // line circom 32
Fr_eq(&expaux[0],&expaux[1],&circuitConstants[1]); // line circom 32
...
uint csoffset = mySignalStart+3736;
...
uint csoffset = mySignalStart+2248;
uint aux_dimensions[1] = {4};
for (uint i = 0; i < 4; i++) {
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[8]); // line circom 124
while(Fr_isTrue(&expaux[0])){
{
...
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[1])) + 11)]].signalStart + 1]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
Mimc2_2_run(mySubcomponents[cmp_index_ref],ctx);
...
{
uint cmp_index_ref = ((1 * (Fr_toInt(&lvar[1]) + 1)) + 6);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
// end load src
...
{
char* temp = Fr_element2str(&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * (Fr_toInt(&lvar[1]) + 1)) + 6)]].signalStart + 0]);
printf("%s",temp);
delete [] temp;
}
{
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[10]].signalStart + 0],&signalValues[mySignalStart + 0]); // line circom 148
